...
- **M7 — Default-flip + fallback + prod enablement. ✅ DONE 2026-06-19.** Decisions taken: **Debug stays on the main thread** (no RPC port) — interactive stepping gets no benefit from real sleep, and async-debug on the main thread was just fixed; Run uses the worker, Debug uses the main-thread interp (the editor is re-evaluated each time, so the state split is moot). **Default flipped:** `workerEvalEnabled()` now returns true whenever the page is cross-origin isolated (SAB available); `?no-worker` opts out to the main-thread path. **Prod enabled:** added COOP/COEP to `playground/vercel.json` (no-regression — CORS satisfies require-corp) so the worker path is live on sema.run after deploy. *Accept:* new e2e `?no-worker forces the main-thread fallback` (2s sleep completes instantly on main thread); full e2e suite now exercises the worker path by default (test server sends COOP/COEP). Deploy: `make playground-deploy`.
