...
# Consumers use the vendored bundle.
#
...
# `cd ui && npm run build && cp` on every invocation.
#
# SPLIT NOTE: the library now also lives standalone at sema-lisp/ui, publishing
# to npm as @sema-lang/ui. While `ui/` is still in this repo the local build
# below (stage 1) produces the bundle. Once `ui/` is removed from the mono, the
# ONLY change here is stage 1: replace the `file ui/dist/sema-ui.js: ui/src/**`
# build recipe with `npm install @sema-lang/ui@{{ui_version}}` and point the
# vendor sources at node_modules/@sema-lang/ui/dist/sema-ui.js. The stage-2
# copies into notebook/playground are unaffected.
ui_version = "0.1.0"
